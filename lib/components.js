...
export function createNeoButton(id, text, iconName, type = 'secondary', onClick) {
    ...
    if (type === 'primary') {
        ...
        btn.style.cssText = 'background: #fff; color: var(--text-main); border: 1px solid var(--btn-secondary-border); border-radius: 10px; padding: 12px; font-weight: 600; display: grid; place-items: center; gap: 8px; width: 100%;';
    }
...
export function createProjectCard(proj) {
    ...
    card.style.cssText = 'background: #fff; border: 1px solid #E5E7EB; border-radius: 12px; padding: 16px; margin-bottom: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.02); display: grid; gap: 8px; position: relative; cursor: pointer; transition: transform 0.2s, box-shadow 0.2s;';
...
export function createTransactionRow(tx, isProjectContext = false, onEdit) {
    ...
    row.style.cssText = 'padding: 16px; background: #fff; border-bottom: 1px solid #F1F5F9; display: grid; grid-template-columns: 40px 1fr auto; gap: 12px; align-items: center;';
...
export const renderBottomNav = (currentView = 'chat') => {
  // 重複防止
  ...
  document.querySelector('.neo-bottom-nav').addEventListener('click', (e) => {
    ...
    if (view) {
      ...
      if (typeof window.switchView === 'function') {
        const targetId = view === 'home' ? 'view-dash' : 'view-' + view;
