...
export async function understandIntent(text, hasImage, contextData) {
    ...
    /** 「新規フォルダ」指定時に同名が既にあると挿入されず別フォルダに落ちるのを防ぐ */
    const _ensureUniqueProjectNameForNewFolder = (base) => {
        if (typeof window.mockDB === 'undefined' || !Array.isArray(window.mockDB.projects)) {
            return String(base || '').trim() || `案件_${new Date().toISOString().slice(0, 10)}`;
        }
        const names = new Set(window.mockDB.projects.map((p) => (p.name || '').trim()).filter(Boolean));
        let n = String(base || '').trim();
        if (!n) n = `案件_${new Date().toISOString().slice(0, 10)}`;
    ...
    // --- Layer 0.95: 新規フォルダ/プロジェクト作成 + 経費（経費単独より先。無いと ADD_EXPENSE だけになり currentOpenProjectId に落ちる） ---
    const _newFolderSignal =
        /新規フォルダ|フォルダを?新規|新規で(?:フォルダ|プロジェクト)|(?:フォルダ|プロジェクト|現場)(?:を)?(?:新規)?(?:作って|作成して|立ち上げて|開始)/;
    if (_EXPENSE_SIGNAL && !_questionExpense && _newFolderSignal.test(text)) {
        const lineItems = _extractLayer1ExpenseLineItems(text);
        if (lineItems.length >= 1) {
            let projectName = '';
            const quoted = text.match(/「([^」]+)」/);
            if (quoted && quoted[1]) {
                projectName = quoted[1].trim();
            }
            if (!projectName && typeof window.extractTags === 'function') {
                const tags = window.extractTags(text);
                if (tags && tags.projectName && String(tags.projectName).trim()) {
                    projectName = String(tags.projectName).trim();
                }
            }
            if (!projectName && typeof window.parseCommand === 'function') {
                const cmd = window.parseCommand(text);
                const t = (cmd && cmd.title && String(cmd.title).trim()) || '';
                if (t.length > 1 && !/^(無題|経費|新規プロジェクト)$/.test(t)) {
                    projectName = t.slice(0, 40);
                }
            }
            if (!projectName) {
                const m = text.match(/(?:で|、)\s*([^、。]{2,24}?)(?:、|。|経費)/);
                if (m && m[1]) projectName = m[1].replace(/\s+/g, '').trim().slice(0, 40);
            }
            if (!projectName) {
                projectName = `案件_${new Date().toISOString().slice(0, 10)}`;
            }

            projectName = _ensureUniqueProjectNameForNewFolder(projectName);

            const amounts = lineItems.map((row) => ({ label: row.label, value: row.amount }));
            const tagsForMeta = typeof window.extractTags === 'function' ? window.extractTags(text) : null;
            return [
                {
                    action: 'CREATE_PROJECT',
                    project_name: projectName,
                    location: tagsForMeta?.location || null,
                    date: tagsForMeta?.date || null,
                    is_silent: true
                },
                {
                    action: 'ADD_EXPENSE',
    ...
    // --- Layer 1.0: 経費のみ（複数明細を高速に ADD_EXPENSE へ） ---
    ...
    const docMatch = text.match(/(?:「([^」]+)」|([^\s]+?))の(請求書|見積書|見積もり|書類)(?:を作って|作成して|作って|プレビュー|生成して|生成|つくりたい|作りたい|ほしい|作成|お願い).*/);
    if (docMatch) {
    ...
    // 5.1 Generic Document Intent (例:「請求書作って」「見積もり作成」)
    const hasDocKeyword = /(請求書|インボイス|見積書|見積もり|領収書|納品書|書類)/.test(text);
    const hasDocVerb = /(作って|作成|発行|生成|出して|作る|お願い|作成して|つくりたい|作りたい|ほしい)/.test(text);
    if (hasDocKeyword && hasDocVerb) {
        let docType = 'invoice';
        if (/(見積書|見積もり)/.test(text)) docType = 'estimate';
        else if (/領収書/.test(text)) docType = 'receipt';
        else if (/納品書/.test(text)) docType = 'delivery';
        return [{
    ...
    try {
        ...
        const hasLayer2DocFallback =
            intents.every(i => i.action === 'UNKNOWN') &&
            /(請求書|インボイス|見積書|見積もり|領収書|納品書|書類)/.test(text) &&
            /(作って|作成|発行|生成|出して|作る|お願い)/.test(text);
        if (hasLayer2DocFallback) {
            let docType = 'invoice';
            if (/(見積書|見積もり)/.test(text)) docType = 'estimate';
            else if (/領収書/.test(text)) docType = 'receipt';
            else if (/納品書/.test(text)) docType = 'delivery';
            intents = [{ action: 'GENERATE_DOCUMENT', document_type: docType, project_name: null, is_silent: true }];
        }
        ...
        if (msg.includes("No API Key") || msg.includes("NO_API_KEY")) {
            return [{ action: "UNKNOWN_ERROR", errorType: "NO_API_KEY" }];
        }
        if (msg.includes("INVALID_API_KEY") || /expired|API_KEY_INVALID|leaked|reported as leaked/i.test(msg)) {
            return [{ action: "UNKNOWN_ERROR", errorType: "INVALID_API_KEY" }];
        }
