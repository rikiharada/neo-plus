...
export async function neoHardResetKnowledge() {
    ...
    try {
        ...
        console.log(`${label} Initiating isolated clean-up for user: ${uid}`);

        // Delete child tables first to avoid FK constraint errors, using strict user_id RLS scoping
        const { error: e1 } = await client.from('activities').delete().eq('user_id', uid);
        if (e1 && !isMissingColumnError(e1)) console.warn(`${label} activities deletion failed:`, e1.message);

        const { error: e2 } = await client.from('documents').delete().eq('user_id', uid);
        if (e2 && !isMissingColumnError(e2)) console.warn(`${label} documents deletion failed:`, e2.message);

        const { error: e3 } = await client.from('files').delete().eq('user_id', uid);
        if (e3 && !isMissingColumnError(e3)) console.warn(`${label} files deletion failed:`, e3.message);

        // Finally delete projects
        const { error: e6 } = await client.from('projects').delete().eq('user_id', uid);
        if (e6 && !isMissingColumnError(e6)) console.warn(`${label} projects deletion failed:`, e6.message);
