 ...
 *
 * 注意: `.eq('user_id', uid)` だけでは user_id が NULL のレガシー行は 0 件マッチし、リモートに残る。
 *       RLS で SELECT できる project id を列挙し、子テーブルは project_id で削除してから projects を消す。
 */
...
/** 知識系 localStorage のみ削除（Supabase の知識テーブルは呼び出し側で触らない） */
...
export async function neoHardResetKnowledge() {
    ...
    try {
        ...
        if (!uid) {
            console.warn(`${label} 未ログイン → ローカルのみクリア（リモートの user_id 指定削除は不可）`);
        ...
        if (projErr) {
            console.warn(`${label} projects 一覧取得:`, projErr.message);
        }
        const projectIds = (projs || []).map((p) => p.id).filter((id) => id != null);

        console.log(`${label} カスケード削除開始（見えている projects: ${projectIds.length} 件）+ user_id=${uid} の行`);
        ...
        for (const pid of projectIds) {
            ...
            if (e1b) console.warn(`${label} transactions project_id=${pid}:`, e1b.message);
        ...
        if (e3) console.warn(`${label} activities user_id:`, e3.message);

        const { error: e4 } = await client.from('transactions').delete().eq('user_id', uid);
        if (e4) console.warn(`${label} transactions user_id:`, e4.message);
        ...
        if (e7) console.warn(`${label} projects user_id:`, e7.message);

        const { data: rpcData, error: rpcErr } = await client.rpc('day_one_wipe_user_data_via_owner');
        if (!rpcErr && rpcData) {
            console.log(`${label} RPC 補助（DEFINER）:`, rpcData);
        } else if (rpcErr) {
            const missing = /function|does not exist|schema cache|PGRST202/i.test(String(rpcErr.message || rpcErr.code || ''));
            if (!missing) console.warn(`${label} RPC day_one_wipe_user_data_via_owner:`, rpcErr.message);
        }

        console.log(
            `${label} リモート DELETE 試行済み。再同期後も remote= が残る場合は Supabase SQL で activities の user_id / project_id を確認してください。`
        );
        ...
        if (typeof window.refreshNeoUserDataFromRemote === 'function') {
            ...
            console.log(`${label} 再同期後、Activities merged の remote= が 0 付近であることを確認してください。`);
        } else {
            console.warn(`${label} window.refreshNeoUserDataFromRemote 未定義 — 再読み込みしてください。`);
        }
